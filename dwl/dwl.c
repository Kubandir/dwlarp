...
static void update_resize_preview(double cur_x, double cur_y);
static void update_move_preview(double cur_x, double cur_y);
...
static struct wlr_scene_rect *resize_preview_v; /* vertical line for H-axis BSP resize */
static struct wlr_scene_rect *resize_preview_h; /* horizontal line for V-axis BSP resize */
static struct wlr_scene_tree *move_preview;     /* drop-target rect shown while dragging */
static struct wlr_scene_rect *move_preview_bg;
static struct wlr_scene_rect *move_preview_border[4]; /* top, bottom, left, right */
...
static const char *grab_cursor_name; /* cursor to keep showing during a grab */
		...
		double dx_unaccel, double dy_unaccel)
{
	...
	if (cursor_mode == CurMove) {
		...
		if (grabc && grabc->isfloating) {
			...
			if (grabc->was_tiled)
				update_move_preview(cursor->x, cursor->y);
			return;
		}
	} else if (cursor_mode == CurResize) {
		if (tiled && resizing_from_mouse) {
			/* resize applied on button release via finish_mouse_resize */
			update_resize_preview(cursor->x, cursor->y);
...
focus:
	/* during a grab keep our cursor and don't re-focus surfaces */
	if (grab_cursor_name) {
		wlr_cursor_set_xcursor(cursor, cursor_mgr, grab_cursor_name);
		return;
	}
...
moveresize(const Arg *arg)
{
	...
	if (grabc->was_tiled) {
		...
		case CurMove:
			...
			break;
		case CurResize: {
			/* BSP-aware snap: corner if both edges are resizable, edge midpoint
			 * if only one is, cancel if neither (window has no neighbor in this quadrant) */
			double ex, ey;
			const char *xc;
			if (!compute_resize_snap(grabc, cursor->x, cursor->y, &ex, &ey, &xc)) {
				cursor_mode = CurNormal;
				grabc = NULL;
				return;
			}
			grab_cursor_name = xc;
			wlr_cursor_warp_closest(cursor, NULL, ex, ey);
			resize_last_update_x = ex;
			resize_last_update_y = ey;
			resizing_from_mouse = 1;
			update_resize_preview(ex, ey);
			if (resize_h_ok)
				wlr_scene_node_set_enabled(&resize_preview_v->node, 1);
			if (resize_v_ok)
				wlr_scene_node_set_enabled(&resize_preview_h->node, 1);
			break;
		}
		}
	} else {
		...
		case CurResize:
			...
			grab_cursor_name = "se-resize";
			break;
		}
	}
	if (grab_cursor_name) {
		wlr_cursor_set_xcursor(cursor, cursor_mgr, grab_cursor_name);
		/* drop pointer focus so clients can't override our cursor mid-grab */
...
/* Render the resize preview lines based on current cursor and the anchor
 * captured at drag start. Each line is anchored to the opposite corner of
 * the grabbed window: for a corner drag the two lines meet at the cursor
 * forming the new corner; for an edge drag the line spans the full window
 * dimension on the perpendicular axis like a moved border. */
void
update_resize_preview(double cur_x, double cur_y)
{
	int x = (int)round(cur_x);
	int y = (int)round(cur_y);
	int half = (int)(resizepreviewpx / 2);
	int t, vy, vh, hx, hw;
	...
	if (resize_h_ok) {
		if (resize_v_ok) {
			...
			vy = resize_window_geom.y;
			vh = resize_window_geom.height;
		}
		wlr_scene_rect_set_size(resize_preview_v, (int)resizepreviewpx, vh);
		wlr_scene_node_set_position(&resize_preview_v->node, x - half, vy);
	}
	...
	if (resize_v_ok) {
		if (resize_h_ok) {
			...
			hx = resize_window_geom.x;
			hw = resize_window_geom.width;
		}
		wlr_scene_rect_set_size(resize_preview_h, hw, (int)resizepreviewpx);
		wlr_scene_node_set_position(&resize_preview_h->node, hx, y - half);
	}
}

/* Show a translucent rect previewing where a dragged tiled window will be
 * inserted into the BSP tree on release. Mirrors insert_client logic:
 * vertical split if the target is wider, horizontal otherwise. The new
 * window claims the half on the cursor's side. */
void
update_move_preview(double cur_x, double cur_y)
{
	Client *target;
	struct wlr_box t, p;
	int wider, bw;
	...
	target = xytoclient(cur_x, cur_y);
	if (!target || target == grabc || target->isfloating || target->isfullscreen) {
		wlr_scene_node_set_enabled(&move_preview->node, 0);
		return;
	}

	t = target->geom;
	wider = t.width >= t.height;
	if (wider) {
		p.y = t.y;
		p.height = t.height;
		p.width = t.width / 2;
		p.x = (cur_x <= t.x + t.width / 2.0) ? t.x : t.x + t.width / 2;
	} else {
		p.x = t.x;
		p.width = t.width;
		p.height = t.height / 2;
		p.y = (cur_y <= t.y + t.height / 2.0) ? t.y : t.y + t.height / 2;
	}

	bw = (int)movepreviewbw;
	wlr_scene_rect_set_size(move_preview_bg, p.width, p.height);
	wlr_scene_node_set_position(&move_preview_bg->node, 0, 0);
	wlr_scene_rect_set_size(move_preview_border[0], p.width, bw);
	wlr_scene_node_set_position(&move_preview_border[0]->node, 0, 0);
	wlr_scene_rect_set_size(move_preview_border[1], p.width, bw);
	wlr_scene_node_set_position(&move_preview_border[1]->node, 0, p.height - bw);
	wlr_scene_rect_set_size(move_preview_border[2], bw, p.height);
	wlr_scene_node_set_position(&move_preview_border[2]->node, 0, 0);
	wlr_scene_rect_set_size(move_preview_border[3], bw, p.height);
	wlr_scene_node_set_position(&move_preview_border[3]->node, p.width - bw, 0);
...
static void
log_filter(enum wlr_log_importance importance, const char *fmt, va_list args)
{
	...
	for (i = 0; i < LENGTH(suppress); i++) {
		if (strstr(buf, suppress[i]))
			return;
	}
...
setup(void)
{
	...
	wlr_log_init(log_level, log_filter);
	...
	resize_preview_v = wlr_scene_rect_create(layers[LyrOverlay], 0, 0, resizepreviewcolor);
	wlr_scene_node_set_enabled(&resize_preview_v->node, 0);
...
tile(Monitor *m)
{
	...
	ox = m->w.x + gappx; oy = m->w.y + gappx;
	ow = m->w.width - 2 * gappx; oh = m->w.height - 2 * gappx;
	if (n > m->nmaster)
		mw = m->nmaster ? (int)roundf(ow * m->mfact) : 0;
	else
		mw = ow;
	i = my = ty = 0;
	wl_list_for_each(c, &clients, link) {
		...
		if (i < m->nmaster) {
			resize(c, (struct wlr_box){
				.x = ox, .y = oy + my, .width = mw,
				.height = (oh - my) / (MIN(n, m->nmaster) - i) - (i < (int)m->nmaster - 1 ? gappx : 0)
			}, 0);
			my += c->geom.height + gappx;
		} else {
			resize(c, (struct wlr_box){
				.x = ox + mw + (m->nmaster ? gappx : 0), .y = oy + ty,
				.width = ow - mw - (m->nmaster ? gappx : 0),
				.height = (oh - ty) / (n - i) - (i < n - 1 ? gappx : 0)
			}, 0);
