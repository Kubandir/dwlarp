 ...
 *   4. on Enter                fork twl-lock-helper (PAM) once; pipe pw → stdin
...
/* Render one lock surface from the shared input/wrong state. */
static void lock_render(Widget *w) {
    ...
    const Font *f = &font_large;
    int tw = text_width(f, LOCK_PROMPT);
    int prompt_x = (W - tw) / 2;
    int prompt_y = H / 2 - f->line_h;
    draw_text(s->px, W, H, prompt_x, prompt_y, f, LOCK_PROMPT, LOCK_FG);

    uint32_t ring = ls.wrong ? LOCK_RING_WRONG : LOCK_RING;

    int n = utf8_count(ls.input, ls.input_len);
    if (n > 16) n = 16;
    int dot = 12, gap = 6;
    int total = n > 0 ? (n * dot + (n - 1) * gap) : dot;
    int dx = (W - total) / 2;
    int dy = prompt_y + f->line_h + 12;
    if (n == 0) {
        fill_rect(s->px, W, H, dx, dy + dot/2 - 1, dot, 2, LOCK_DIM);
    }
    for (int i = 0; i < n; i++) {
        fill_rect(s->px, W, H, dx + i * (dot + gap), dy, dot, dot, ring);
    }

    /* Caps-lock hint: small yellow "CAPS" tag below the dot row so users
     * notice before mistyping a long password. */
    if (xkb_caps_on) {
        const Font *cf = &font_small;
        int cw = text_width(cf, "CAPS");
        int cx = (W - cw) / 2;
        int cy = dy + dot + 10;
        draw_text(s->px, W, H, cx, cy, cf, "CAPS", LOCK_CAPS);
    }
