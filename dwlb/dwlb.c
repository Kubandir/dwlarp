...
typedef struct {
	...
	CustomText title, status, leftstatus;
...
draw_frame(Bar *bar)
{
	...
	/* Left status (logo/time/date) drawn before tags */
	if (bar->leftstatus.text[0]) {
		uint32_t ls_width = TEXT_WIDTH(bar->leftstatus.text, bar->width, bar->textpadding);
		x = draw_text(bar->leftstatus.text, 0, y, foreground, foreground_mask,
			      background, &inactive_fg_color, &inactive_bg_color,
			      bar->width, bar->height, bar->textpadding,
			      bar->leftstatus.colors, bar->leftstatus.colors_l);
		(void)ls_width;
	}
...
pointer_frame(void *data, struct wl_pointer *pointer)
{
	...
	uint32_t x = 0, i = 0;
	if (seat->bar->leftstatus.text[0])
		x = TEXT_WIDTH(seat->bar->leftstatus.text, seat->bar->width, seat->bar->textpadding) / buffer_scale;
	if (seat->pointer_x < x) {
...
read_socket(void)
{
	...
	} else if (!strcmp(wordbeg, "leftstatus")) {
		if (!*wordend)
			return;
		if (all) {
			Bar *first = NULL;
			wl_list_for_each(bar, &bar_list, link) {
				if (first) {
					copy_customtext(&first->leftstatus, &bar->leftstatus);
				} else {
					parse_into_customtext(&bar->leftstatus, wordend);
					first = bar;
				}
				bar->redraw = true;
			}
...
main(int argc, char **argv)
{
	...
	for (; i < argc; i++) {
		if (!strcmp(argv[i], "-status")) {
			...
			free(status);
			return 0;
		} else if (!strcmp(argv[i], "-leftstatus")) {
			if (++i + 1 >= argc)
				DIE("Option -leftstatus requires two arguments");
			client_send_command(&sock_address, argv[i], "leftstatus", argv[i + 1], target_socket);
			return 0;
		} else if (!strcmp(argv[i], "-leftstatus-stdin")) {
			if (++i >= argc)
				DIE("Option -leftstatus-stdin requires an argument");
			char *status = malloc(TEXT_MAX * sizeof(char));
			while (fgets(status, TEXT_MAX-1, stdin)) {
				status[strlen(status)-1] = '\0';
				client_send_command(&sock_address, argv[i], "leftstatus", status, target_socket);
			}
			free(status);
			return 0;
	...
	{
		/* Split fontstr on commas so fcft can fall back across families */
		const char *names[8];
		size_t nnames = 0;
		char *fdup = strdup(fontstr);
		if (!fdup) DIE("oom");
		char *p = fdup;
		while (p && nnames < 8) {
			char *c = strchr(p, ',');
		...
		if (!nnames || !(font = fcft_from_name(nnames, names, buf)))
			DIE("Could not load font");
		/* fdup leaked intentionally — names[] points into it for font lifetime */
	}
